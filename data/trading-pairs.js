...
const helpers = {
   ...
   * @param {string} pair - El par de trading
   * @returns {number} El valor del pip
   */
  getPipValue: (pair) => tradingPairs[pair]?.valorPip,

  /**
   * Obtiene las unidades por lote para un par específico
   * @param {string} pair - El par de trading
   * @returns {number} Las unidades por lote
   */
  getUnits: (pair) => tradingPairs[pair]?.unidades,

  /**
   * Obtiene el número de decimales para un par específico
   * @param {string} pair - El par de trading
   * @returns {number} El número de decimales
   */
  getDecimals: (pair) => tradingPairs[pair]?.decimales,

  /**
   * Obtiene el tamaño del pip basado en los decimales
   * @param {string} pair - El par de trading
   * @returns {string} El tamaño del pip como string decimal
   */
  getPipSize: (pair) => {
...
// Si estamos en un entorno que soporta módulos ES
if (typeof exports !== "undefined") {
  exports.tradingPairs = tradingPairs;
  exports.helpers = helpers;
} else {
  window.tradingPairs = tradingPairs;
  window.tradingHelpers = helpers;
}
